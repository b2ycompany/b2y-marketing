...
export default function SettingsPage() {
    ...
    // No futuro, este estado virá de uma API que checa as conexões no Firestore
    const [connections, setConnections] = useState({ meta: true, google: false });
            ...
            <div className="w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-dark-card border border-gray-700 rounded-xl p-6 flex flex-col justify-between">
                        ...
                        <div className="mt-4">{connections.meta ? (<div className="flex items-center space-x-2 text-sm font-semibold text-green-400"><LinkIcon size={16}/><span>Conectado</span></div>) : (<button className="w-full font-semibold text-sm bg-primary text-white py-2 px-4 rounded-lg hover:opacity-90">Conectar</button>)}</div>
                    </div>
                    ...
                    <div className="bg-dark-card border border-gray-700 rounded-xl p-6 flex flex-col justify-between">
                        ...
                        <div className="mt-4">{connections.google ? (<div className="flex items-center space-x-2 text-sm font-semibold text-green-400"><LinkIcon size={16}/><span>Conectado</span></div>) : (<button onClick={handleGoogleConnect} className="w-full font-semibold text-sm bg-primary text-white py-2 px-4 rounded-lg hover:opacity-90">Conectar</button>)}</div>
                    </div>
