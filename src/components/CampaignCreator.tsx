...
export default function CampaignCreator() {
    ...
    const fetchInitialData = async () => {
        ...
        try {
            ...
            // Vamos ver no console do navegador o que está dentro de 'pagesData'
            console.log("DADOS DAS PÁGINAS RECEBIDOS DA API:", pagesData);
    ...
    // As outras funções handlers continuam aqui...
    const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>, formSetter: React.Dispatch<React.SetStateAction<any>>) => { formSetter((prev: any) => ({ ...prev, [e.target.name]: e.target.value })); };
    const handleCreateCampaign = async (e: React.FormEvent) => { e.preventDefault(); if (!user) return; setIsSubmitting(prev => ({ ...prev, campaign: true })); setFeedback(null); setCreatedIds({ campaignId: '', adSetId: '' }); try { const idToken = await user.getIdToken(true); const res = await fetch('/api/facebook/campaigns/create', { method: 'POST', headers: { 'Authorization': `Bearer ${idToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify(campaignForm) }); const result = await res.json(); if (!res.ok) throw new Error(result.error); setFeedback({ type: 'success', message: `Passo 1 OK! Campanha criada com ID: ${result.campaignId}` }); setCreatedIds(prev => ({ ...prev, campaignId: result.campaignId })); } catch (error: any) { setFeedback({ type: 'error', message: error.message }); } finally { setIsSubmitting(prev => ({ ...prev, campaign: false })); } };
    const handleCreateAdSet = async (e: React.FormEvent) => { e.preventDefault(); if (!user || !createdIds.campaignId) return; setIsSubmitting(prev => ({ ...prev, adSet: true })); setFeedback(null); try { const idToken = await user.getIdToken(true); const budgetInCents = Math.round(parseFloat(adSetForm.dailyBudget.replace(',', '.')) * 100); const payload = { adSetName: adSetForm.adSetName, targetingCountry: adSetForm.targetingCountry, dailyBudget: budgetInCents, adAccountId: campaignForm.adAccountId, campaignId: createdIds.campaignId }; const res = await fetch('/api/facebook/adsets/create', { method: 'POST', headers: { 'Authorization': `Bearer ${idToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); const result = await res.json(); if (!res.ok) throw new Error(result.error); setFeedback({ type: 'success', message: `Passo 2 OK! Conjunto de Anúncios criado com ID: ${result.adSetId}` }); setCreatedIds(prev => ({ ...prev, adSetId: result.adSetId })); } catch (error: any) { setFeedback({ type: 'error', message: error.message }); } finally { setIsSubmitting(prev => ({ ...prev, adSet: false })); } };
    const handleCreateAd = async (e: React.FormEvent) => { e.preventDefault(); if (!user || !createdIds.adSetId) return; setIsSubmitting(prev => ({...prev, ad: true})); setFeedback(null); try { const idToken = await user.getIdToken(true); const payload = { ...adForm, adAccountId: campaignForm.adAccountId, adSetId: createdIds.adSetId }; const res = await fetch('/api/facebook/ads/create', { method: 'POST', headers: { 'Authorization': `Bearer ${idToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); const result = await res.json(); if (!res.ok) throw new Error(result.error); setFeedback({ type: 'success', message: `GRANDE FINAL! Anúncio criado com ID: ${result.adId}. Sua campanha está pronta (e pausada) no Gerenciador!` }); } catch (error: any) { setFeedback({ type: 'error', message: error.message }); } finally { setIsSubmitting(prev => ({...prev, ad: false})); } };
            ...
            <form onSubmit={handleCreateCampaign} className="p-8 bg-dark-card rounded-xl shadow-lg space-y-6 border border-gray-700">
                ...
                <div>
                    ...
                    <select name="objective" value={campaignForm.objective} onChange={(e) => handleFormChange(e, setCampaignForm)} className="mt-1 block w-full bg-gray-900 border-gray-600 rounded-md py-2 pl-3 pr-10">
                        <option value="OUTCOME_TRAFFIC">Tráfego</option><option value="OUTCOME_ENGAGEMENT">Engajamento</option><option value="OUTCOME_LEADS">Geração de Cadastros</option>
                    </select>
                </div>
                <button type="submit" disabled={isSubmitting.campaign || createdIds.campaignId !== '' || !!initialDataError} className="w-full font-bold py-2.5 px-4 rounded-lg text-white bg-primary hover:opacity-90 disabled:bg-gray-500 disabled:cursor-not-allowed transition">
            ...
            {createdIds.campaignId && (
                <form onSubmit={handleCreateAdSet} className="p-8 bg-dark-card rounded-xl shadow-lg space-y-6 border border-gray-700 animate-fade-in">
                    ...
                    <div>
                        ...
                        <input type="number" name="dailyBudget" value={adSetForm.dailyBudget} onChange={(e) => handleFormChange(e, setAdSetForm)} className="mt-1 block w-full bg-gray-900 border-gray-600 rounded-md px-3 py-2" placeholder="Ex: 10.50" step="0.01" required/>
                        <a href={campaignForm.adAccountId ? `https://www.facebook.com/ads/manager/account_settings/account_billing?act=${campaignForm.adAccountId}` : '#'} target="_blank" rel="noopener noreferrer" className={`text-xs text-gray-400 hover:text-primary mt-1 inline-block ${!campaignForm.adAccountId && 'pointer-events-none opacity-50'}`}>Gerenciar forma de pagamento no Facebook ↗</a>
                    </div>
                ...
                <form onSubmit={handleCreateAd} className="p-8 bg-dark-card rounded-xl shadow-lg space-y-6 border border-gray-700 animate-fade-in">
                    ...
                    <div>
                        ...
                        <select name="pageId" value={adForm.pageId} onChange={(e) => handleFormChange(e, setAdForm)} className="mt-1 block w-full bg-gray-900 border-gray-600 rounded-md py-2 pl-3 pr-10" required>
                            <option value="" disabled>{pages.length > 0 ? "Selecione uma página..." : "Carregando páginas..."}</option>
