...
export default function CampaignCreator() {
    ...
    // Estados para controlar os dados de cada formulário, com valores padrão para robustez
    ...
    const [adSetForm, setAdSetForm] = useState<AdSetFormData>({ adSetName: 'Conjunto de Anúncios - Brasil', dailyBudget: '20.00' });
    const [adForm, setAdForm] = useState<AdFormData>({ adName: 'Anúncio Principal - Oferta de Lançamento', pageId: '', message: 'Conheça nossa solução completa para marketing digital e impulsione seus resultados hoje mesmo.', headline: 'B2Y Marketing - Automação Inteligente', imageUrl: 'https://images.unsplash.com/photo-1557804506-669a67965ba0?q=80&w=1200', link: 'https://b2y-marketing.vercel.app' });
    ...
    const handleNextStep = async () => {
        ...
        try {
            ...
            if (activeStep === 1) {
                if (!campaignForm.campaignName || !campaignForm.adAccountId) { throw new Error("Preencha todos os campos do Passo 1."); }
                ...
                if (!adSetForm.adSetName || !adSetForm.dailyBudget) { throw new Error("Preencha todos os campos do Passo 2."); }
                const budgetInCents = Math.round(parseFloat(adSetForm.dailyBudget.replace(',', '.')) * 100);
                const payload = { ...adSetForm, adAccountId: campaignForm.adAccountId, campaignId: createdIds.campaignId, dailyBudget: budgetInCents, targeting: targeting };
                ...
                if (!adForm.adName || !adForm.pageId || !adForm.message || !adForm.headline || !adForm.link || !adForm.imageUrl) { throw new Error("Preencha todos os campos do Passo 3."); }
